document.addEventListener("DOMContentLoaded", () => {
  ...
  async function procesarParaEspejo(item) {
    try {
      ...
      const esRM_PM_RD = ["RM", "PM", "RD"].includes(op);

      if (!esCajon && !esTM && !esRM_PM_RD) return;
      ...
      const row = {
        Fecha: tsEvent,
        Legajo: legajo,
        Nombre_Matriz: esCajon ? nombreMatriz : (esRM_PM_RD ? `${op} ${matNum}` : item.descripcion),
        Matriz: esCajon ? matNum : (esRM_PM_RD ? matNum : op),
        Uni: uni,
        Premio: premio,
